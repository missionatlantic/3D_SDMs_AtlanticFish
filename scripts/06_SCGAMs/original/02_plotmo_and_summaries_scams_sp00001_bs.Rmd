...
for (i in n){
  ...
  ###Extraigo resumen de analisis para todas las especies
   v_summary<-summary(res$smod[[length(res$smod)]])
   s1<-v_summary$r.sq
   s2<-v_summary$dev.expl
   n_occ <- v_summary[["n"]]/2
   model <- res$smod[[length(res$smod)]]
   var.incl <- as.numeric(vars %in% res$svars) # is the variable included in the final model?
   chisq <- rep(0, length(vars))
   chisq <- as.numeric(v_summary[["chi.sq"]])[match(vars, res$svars)] # match the order from the model with the variables tried

  # abro pdf para guardar plots por especie 
  png (here::here ("data", "derived_data" , "models_sp00001", "plotmos", paste0("plotmo_SP",i, filenames_sp[i],".png")))
  ...
  {
    sink(here::here("data", "derived_data" , "models_sp00001", "summaries", paste0("summary_SP",i, ".txt")))
      print(summary(scgam))
    sink()
  }
...
write.csv(scams_30spp, file=here::here ("data", "derived_data" , "models_sp00001", "summary_scams_30spp.csv"))
```
