...
With this script we generate a brick including no extrapolation areas based on MESS calculation for each depth layer (47 for each species). 
...
for (i in n) {
  ...
  maps_list <-  intersect(
                  list.files(path=here::here("data", "derived_data", "models_sp00001", "MESS"), pattern=paste0("noextrapolation_SP_",i, "_"), all.files=TRUE, full.names=TRUE),
                  list.files(path=here::here("data", "derived_data", "models_sp00001", "MESS"), pattern = "\\.tif$", all.files=TRUE, full.names=TRUE))
  ...
  #stack them 
  SP_MESS_set <- stack(maps)
  
  #save brick
  writeRaster(SP_MESS_set,
            filename=here::here ("data", "derived_data", "models_sp00001", "MESS", "bricks", paste0("MESS_brick_SP",i)),
            format="GTiff",
            overwrite=TRUE,
            options=c("INTERLEAVE=BAND","COMPRESS=LZW"))
  

  # raster_max <- calc(SP_HSM_set, base::max)
  # 
  # # Save raster
  # writeRaster(raster_max,
  #            filename=here::here ("models_sp00001", "rasters_set_by_species", paste0("HSM_MAX_SP_", i,".tif")),
  #            format="GTiff",
  #            overwrite=TRUE)
  # 
  }
